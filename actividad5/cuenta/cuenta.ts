...
class Cuenta{
    ...
    constructor(nombreParam:string, cantidaParam:number, tipoCuentaParam:string, numeroCuentaParam:string){
    ...
    depositar():string{
        let msg = this.cantidad < 5 ? 'Tu deposito debe de ser mayor de $5.00':`Deposito de ${this.cantidad} realizado con exito`;
        return msg;
    }
    ...
    retirar(valor:number):string{
        ...
        if(valor < 5){
            ...
            msg = `No cuentas con fondos suficientes para retirar ${valor}`;
        }else if(this.cantidad == 0){
            msg = "Fondos insuficientes";
        }else{
            this.cantidad -= valor;
            msg = `Haz retirado ${valor}, tu saldo actual es de ${this.cantidad}`;
        }
    ...
    mostrarDatos():string{
        let msg:string = `Nombre: ${this.nombre}\n Tipo de Cuenta: ${this.tipoCuenta}\n N-Cuenta : ${this.numeroCuenta}`;
        return msg;
    }

}

let cuenta: Cuenta = new Cuenta("Juan", 20, "Ahorro", "2w222");
